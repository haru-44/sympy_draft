...
from .factor_ import multiplicity, totient
from .residue_ntheory import _sqrt_mod_prime_power, is_quad_residue
from .sieve_ import FactorSieve

import math

def _pre():
    global _factor
    _factor = FactorSieve(10**5)
...
def _a(n, k, prec):
    ...
    tot_k1 = totient._from_factors(_factor.factorint(k1))
    if p != 2 or e >= 3:
        d1, d2 = gcd(k1, 24), gcd(k2, 24)
        e = 24//(d1*d2)
        tot_k2 = totient._from_factors(_factor.factorint(k2))
        n1 = ((d2*e*n + (k2**2 - 1)//d1)*
            pow(e*k2*k2*d2, tot_k1 - 1, k1)) % k1
        n2 = ((d1*e*n + (k1**2 - 1)//d2)*
            pow(e*k1*k1*d1, tot_k2 - 1, k2)) % k2
        return mpf_mul(_a(n1, k1, prec), _a(n2, k2, prec), prec)
    if e == 2:
        n1 = ((8*n + 5)*pow(128, tot_k1 - 1, k1)) % k1
        n2 = (4 + ((n - 2 - (k1**2 - 1)//8)*(k1**2)) % 4) % 4
        return mpf_mul(mpf_mul(
            from_int(-1),
            _a(n1, k1, prec), prec),
            _a(n2, k2, prec))
    n1 = ((8*n + 1)*pow(32, tot_k1 - 1, k1)) % k1
    n2 = (2 + (n - (k1**2 - 1)//8) % 2) % 2
    return mpf_mul(_a(n1, k1, prec), _a(n2, k2, prec), prec)
